...
export default function ClassDetail() {
  ...
  // State quản lý tab hiển thị: Mặc định luôn là 'files' vì Quizzes đã ra trang riêng
  const [activeTab, setActiveTab] = useState<'files'>('files');
  const [isCourseInfoOpen, setIsCourseInfoOpen] = useState(true);

  const userRole = localStorage.getItem('userRole');

  // Effect lấy dữ liệu lớp học khi component được mount
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="w-[80vw] mx-auto">
        {/* --- Header Section (Thông tin tên lớp, trạng thái) --- */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          {/* --- Body: Thông tin chi tiết (Collapsible) --- */}
          {classData.course && (
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="flex border-b">
            ...
            {/* Nút Quizzes: Chuyển hướng sang trang QuizList */}
            <button
              onClick={() => navigate(`/class/${clid}/quizzes`)}
              className="flex-1 px-6 py-4 font-medium transition-colors text-gray-600 hover:bg-gray-50"
            >
              <div className="flex items-center justify-center gap-2">
          ...
          <div className="p-6">
            ...
            {/* Tab Quizzes đã được loại bỏ nội dung ở đây */}
          </div>
        </div>
      </div>
    </div>
